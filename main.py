...
class Game:
    ...
    def new(self):
        ...
        self.bossdoor =pg.sprite.LayeredUpdates()
        ...
        for tile_object in self.map.tmxdata.objects:
            ...
            if tile_object.name in ['health', 'shotgun', 'talisman1', 'talisman2']:
                if tile_object.name == 'talisman1':
                    if self.draw_piece1:
                        pass
                    else:
                        Item(self, obj_center, tile_object.name)

                elif tile_object.name == 'talisman2':
                    if self.draw_piece2:
                        pass
                    else:
                        Item(self, obj_center, tile_object.name)
            ...
            if tile_object.name == 'bossdoor':
                Bossdoor(self, tile_object.x, tile_object.y,tile_object.width, tile_object.height)
    ...
    def update(self):
        ...
        for hit in hits:
            ...
            if hit.type == 'talisman1':
                ...
                self.piece1_ecran = True

            if hit.type == 'talisman2':
                hit.kill()
                self.bosspieces += 1
                self.draw_piece2 = True
                self.piece2_ecran = True
        ...
        if hit:
            ...
            if self.level_now == 'level2':
                self.level_now = 'level1'
            else:
                self.level_now = 'level2'
            self.save_player()

        hit = pg.sprite.spritecollide(self.player, self.bossdoor, False, collide_wall)
        if hit and self.bosspieces ==2:
            self.playing = False
            self.change_level = True
            self.level_now = 'boss'
            self.save_player()
    ...
    def draw(self):
        ...
        for sprite in self.all_sprites:
            ...
            if isinstance(sprite, Mob) or isinstance(sprite, Player) or isinstance(sprite, Bullet) or isinstance(sprite, Boss):
                if self.draw_debug:
                    ...
                    print (self.camera.apply_rect(sprite.hit_rect))
        ...
        if self.draw_piece2 == True:
            self.draw_pieces(25, 15, self.item_images['talisman2'])
        ...
        if self.info:
            self.screen.blit(self.dim_screen, (0, 0))
            self.draw_text("Information", self.title_font, 65, YELLOW, WIDTH / 2, HEIGHT / 2 - 250,"center")
            self.draw_text("Press N for hard mode", self.title_font, 65, YELLOW, WIDTH / 2 - 475, HEIGHT / 2 - 150, "nw")
            self.draw_text("Press P for paused", self.title_font, 65, YELLOW, WIDTH / 2 - 475, HEIGHT / 2 - 50, "nw")
            self.draw_text("Press H to see collision", self.title_font, 65, YELLOW, WIDTH / 2 - 475, HEIGHT / 2 + 50, "nw")
            self.draw_text("Press V to see position vector", self.title_font, 65, YELLOW, WIDTH / 2 - 475, HEIGHT / 2 + 150, "nw")

        if self.show_vec:
            v = vec(50,0)
            pg.draw.line(self.screen, RED, (int(self.player.pos.x + self.camera.x), int(self.player.pos.y + self.camera.y)), (int(v.rotate(-self.player.rot).x + self.player.pos.x + self.camera.x ), int(v.rotate(-self.player.rot).y + self.player.pos.y + self.camera.y)),5)
